...
function HorItem(props) {
    return (
        <div className='HorItems'>
            ...
            <img style={{height: '55px', width: '55px'}} src={props.description} alt=""/>
        </div>
    );
}
    ...
    return (
        <div>
            ...
            {hourlyData.map((item, index) => (
                <HorItem key={index} time={item.time} temperature={item.temperature} icon={item.icon} description={item.description} />
            ))}
        </div>
    );
}
